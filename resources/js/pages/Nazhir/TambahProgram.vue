...
return;
    ...
    try {
        ...
        await axios.post('/api/nazhir/program', formData, {
            headers: {
  ...
  <NazhirLayout>
    <div class="max-w-3xl mx-auto space-y-6">
      ...
      <div class="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider">
        ...
        <span>/</span>
        <span class="text-gray-700">Tambah Program</span>
      </div>

      <div>
        <h1 class="text-2xl font-black text-gray-900 leading-tight">Tambah Program Wakaf Baru</h1>
        <p class="text-xs text-gray-500">Buat program wakaf baru dengan detail target dana, tenggat tanggal, dan deskripsi lengkap.</p>
      </div>

      <!-- Form Card -->
      <form @submit.prevent="handleSubmit" class="bg-white border border-gray-150 rounded-2xl p-6 md:p-8 shadow-xs space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <!-- Gambar Thumbnail -->
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-bold text-gray-750">Gambar Thumbnail</label>
            <input 
              type="file" 
              accept="image/*"
        ...
        <div v-if="thumbnailPreview" class="border border-gray-200 rounded-xl p-3 bg-gray-50 flex flex-col items-center gap-2 max-w-xs">
          <span class="text-[10px] font-bold text-gray-400 uppercase">Preview Gambar</span>
          <img :src="thumbnailPreview" class="w-full h-32 object-cover rounded-lg" alt="Thumbnail Preview" />
        </div>
