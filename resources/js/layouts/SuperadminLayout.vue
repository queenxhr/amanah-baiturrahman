    ...
    <aside :class="[
      ...
      <div class="flex flex-col flex-1">
        ...
        <div class="mb-8">
          ...
          <span class="text-[10px] uppercase font-black text-gray-300 tracking-wider">Panel Superadmin</span>
        </div>
        ...
        <div class="bg-white/5 border border-white/10 rounded-2xl p-4 mb-8 flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <h4 class="font-bold text-sm text-white break-words whitespace-normal">{{ user?.nama ?? 'Superadmin' }}</h4>
        ...
        <nav class="space-y-2 flex-1">
          <span class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 px-2">Kendali Utama</span>
          
          <Link href="/superadmin/dashboard" 
                @click="closeSidebarOnMobile"
                :class="[
                  'flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition duration-250',
                  $page.url === '/superadmin/dashboard' ? 'bg-[#b1cf49] text-[#143E2C]' : 'text-gray-300 hover:bg-white/5 hover:text-white'
                ]">
          ...
          <Link href="/superadmin/users" 
                ...
                :class="[
                  'flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition duration-250',
                  $page.url === '/superadmin/users' ? 'bg-[#b1cf49] text-[#143E2C]' : 'text-gray-300 hover:bg-white/5 hover:text-white'
                ]">
          ...
          <Link href="/superadmin/programs" 
                ...
                :class="[
                  'flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition duration-250',
                  $page.url === '/superadmin/programs' ? 'bg-[#b1cf49] text-[#143E2C]' : 'text-gray-300 hover:bg-white/5 hover:text-white'
                ]">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
            Verifikasi Konten
          </Link>

          <Link href="/superadmin/pencairan" 
                @click="closeSidebarOnMobile"
                :class="[
                  'flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition duration-250',
                  $page.url === '/superadmin/pencairan' ? 'bg-[#b1cf49] text-[#143E2C]' : 'text-gray-300 hover:bg-white/5 hover:text-white'
                ]">
    ...
    <div class="flex-grow min-w-0 flex flex-col transition-all duration-300" :class="isSidebarOpen ? 'md:pl-64' : 'pl-0'">
      ...
      <header class="sticky top-0 bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6 z-20 shadow-sm">
        <div class="flex items-center gap-3">
          ...
          <span class="hidden md:inline text-sm font-bold text-gray-700">Panel Superadmin</span>
        </div>
